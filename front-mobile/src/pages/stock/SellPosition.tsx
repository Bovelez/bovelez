...
import { AlertTriangle, ArrowDownRight, ArrowUpRight, ArrowLeft, CheckCircle, Loader2 } from "lucide-react";
...
export default function SellPosition() {
  ...
  if (isLoading) {
    return (
      <div data-testid="sell-loading" className="flex min-h-screen items-center justify-center">
  ...
  if (!price || !company) {
    return (
      <div data-testid="sell-error" className="flex min-h-screen flex-col items-center justify-center gap-4 px-6">
        ...
        <button data-testid="sell-back-btn" onClick={() => navigate(-1)}
  ...
  if (confirmed) {
    return (
      <div data-testid="sell-confirmed" className="flex min-h-screen items-center justify-center p-6" style={{ fontFamily: "var(--font-body)" }}>
        <div className="w-full max-w-sm text-center">
          <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-5 bg-emerald-500/15">
            <CheckCircle size={36} className="text-emerald-400" />
          </div>
          <h2 className="text-emerald-400 mb-1 text-2xl font-bold">Venta registrada</h2>
          ...
          <button data-testid="sell-go-portfolio" onClick={() => navigate("/app/portfolio")}
  ...
  return (
    <div data-testid="sell-page" className="min-h-screen pb-8" style={{ fontFamily: "var(--font-body)" }}>
      {/* Header */}
      <div className="flex items-center gap-3 px-4 pt-5 pb-4 border-b border-[var(--border)]">
        <button data-testid="sell-back-btn" onClick={() => navigate(-1)} className="p-2 rounded-xl bg-[var(--surface)]">
          <ArrowLeft size={18} className="text-[var(--text)]" />
        </button>
        <div>
          <p className="font-semibold text-[var(--text)]">Vender {company.ticker}</p>
          <p className="text-xs text-[var(--text-muted)]">Tenés {maxQty} acciones · Precio: ${sellPrice.toFixed(2)}</p>
        </div>
      </div>

      <div className="px-4 pt-5 space-y-4">
        {maxQty === 0 && (
          <div className="flex items-center gap-3 p-4 rounded-xl border" style={{ backgroundColor: "var(--primary-soft)", borderColor: "rgba(255,107,53,0.35)" }}>
            <AlertTriangle size={18} className="text-[var(--primary)]" />
            <p className="text-sm font-semibold text-[var(--primary)]">No tenés acciones de {company.ticker}.</p>
          </div>
        )}

        {isLoss && maxQty > 0 && (
          <div data-testid="sell-loss-warning" className="flex items-center gap-3 p-4 rounded-xl border" style={{ backgroundColor: "var(--primary-soft)", borderColor: "rgba(255,107,53,0.35)" }}>
            <AlertTriangle size={18} className="text-[var(--primary)]" />
            <p className="text-sm font-semibold text-[var(--primary)]">Vendiendo con pérdida del {Math.abs(pnlPct).toFixed(2)}%.</p>
          </div>
        )}
        ...
        <div className="p-5 rounded-2xl bg-[var(--surface)] border border-[var(--border)]">
          ...
          <input data-testid="sell-qty-slider" type="range" min={1} max={Math.max(1, maxQty)} value={qty}
            onChange={(e) => setQty(parseInt(e.target.value))}
            className="w-full accent-[var(--primary)] mb-3" disabled={maxQty === 0} />
          <div className="flex justify-between items-center">
            <span className="text-xs text-[var(--text-faint)]">1 acción</span>
            <span data-testid="sell-qty-display" className="font-bold font-mono text-lg text-[var(--text)]">{qty}</span>
            <span className="text-xs text-[var(--text-faint)]">{maxQty} acciones</span>
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-[var(--surface)] border border-[var(--border)]">
          ...
          <input data-testid="sell-date-input" type="date" value={date} onChange={(e) => setDate(e.target.value)}
        ...
        {/* Summary */}
        <div className={`p-5 rounded-2xl border ${isLoss ? "bg-rose-500/10 border-rose-500/20" : "bg-emerald-500/10 border-emerald-500/20"}`}>
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between pt-1 border-t border-white/10">
              <span className="text-sm text-[var(--text-muted)]">G/P realizada</span>
              <span data-testid="sell-pnl" className={`text-sm font-bold flex items-center gap-1 font-mono ${isLoss ? "text-rose-400" : "text-emerald-400"}`}>
        ...
        {errorMsg && (
          <div data-testid="sell-error-msg" className="flex items-center gap-2 p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-sm">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button data-testid="sell-confirm-btn" onClick={handleConfirm}
            ...
            {sellMutation.isPending ? <Loader2 size={15} className="animate-spin" /> : null}
