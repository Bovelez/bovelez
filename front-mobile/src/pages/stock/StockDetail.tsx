...
export default function StockDetail() {
  ...
  const isInWatchlist = (watchlistQuery.data ?? []).some(
  ...
  const toggleWatchlist = () => {
    if (!normalizedTicker || isWatchlistPending) return;
    if (isInWatchlist) removeWatchlistItem.mutate(normalizedTicker);
    else addWatchlistItem.mutate(normalizedTicker);
  };
  ...
  return (
    <div
      ...
      className="min-h-screen bg-[var(--bg-deep)] pb-24"
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 pt-5 pb-4 border-b border-[var(--border)] bg-[var(--surface)]">
        <button
          data-testid="stock-back-btn"
          onClick={() => navigate(-1)}
          className="p-2 rounded-xl bg-[var(--surface-2)]"
        >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span
              className="font-mono font-black text-base text-[var(--text)]"
        ...
        {price && (
          <p
            ...
            className="font-mono font-bold text-xl text-[var(--text)]"
          >
      ...
      <div className="px-4 pb-4">
        <button
          ...
          className="w-full py-3 rounded-xl text-sm font-semibold border border-[var(--border-strong)] text-[var(--text)] bg-[var(--surface)] disabled:opacity-60"
        >
          {watchlistQuery.isLoading
            ? 'Cargando...'
            : isInWatchlist
      ...
      <div
        ...
        className="flex gap-1 mx-4 p-1 rounded-xl bg-[var(--surface-2)] mb-4"
      >
        {TABS.map((t) => (
          <button
            key={t}
            ...
            style={
              tab === t ? { background: 'var(--gradient-brand)' } : undefined
            }
          >
            {t}
          </button>
