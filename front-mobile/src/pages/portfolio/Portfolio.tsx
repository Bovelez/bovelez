...
import { TrendingUp, Wallet, ArrowUpRight, ArrowDownLeft, BriefcaseBusiness, History } from "lucide-react";
    ...
    >
      ...
      <header className="px-4 pt-5 pb-4 flex flex-col gap-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-black text-[var(--text)]">Mi Portfolio</h1>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 flex items-center gap-2 rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-4 py-3">
            ...
            <div>
              <p className="text-[9px] font-bold uppercase tracking-widest text-orange-100/80">Valor</p>
              <p className="font-mono text-lg font-black text-[var(--text)]">
          ...
          {!isLoading && (
            <div
              ...
              style={{
                ...
                borderColor: isPnlPositive ? "rgba(52,211,153,0.2)" : "rgba(251,113,133,0.2)",
              }}
            >
              <div
              ...
              <div>
                <p className="text-[9px] font-bold uppercase tracking-widest text-orange-100/80">Resultado</p>
                <p className="font-mono text-sm font-black" style={{ color: isPnlPositive ? "#34d399" : "#fb7185" }}>
                  {isPnlPositive ? "+" : ""}${Math.abs(totalPnl).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </p>
              </div>
            </div>
          )}
      ...
      >
        ...
        {!isLoading && !portfolioError && positions.length === 0 && (
          <div className="flex flex-col items-center gap-3 py-12 text-center px-4">
            <BriefcaseBusiness size={28} className="text-[var(--text-muted)]" opacity={0.4} />
            <p className="text-sm font-semibold text-[var(--text)]">Sin posiciones abiertas</p>
            <p className="text-xs text-[var(--text-muted)]">Usá el terminal para tu primera compra.</p>
          </div>
        )}

        {!isLoading && !portfolioError && positions.map((position) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-mono text-sm font-black text-[var(--text)]">{position.ticker}</p>
                  <p className="text-[11px] text-[var(--text-muted)]">{position.quantity} acc. · PPC {useMoney(position.avgCost)}</p>
                </div>
              </div>

              <div className="flex items-center gap-3">
                <div className="text-right">
                  <p className="font-mono text-sm font-semibold text-[var(--text)]">{useMoney(position.currentPrice)}</p>
                  {position.pnlPercent !== null ? (
                    <PnlText value={position.pnlPercent} format="percent" data-testid={`position-pct-${position.ticker}`} />
                  ) : (
                    <span className="text-xs text-[var(--text-muted)]">—</span>
                  )}
                </div>
                <span className="text-[var(--text-faint)]">
                  <History size={16} className={isPositive ? "text-emerald-400" : "text-rose-400"} />
                </span>
              </div>
            </div>
          );
