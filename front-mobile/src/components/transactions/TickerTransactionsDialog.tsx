  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          data-testid="ticker-dialog"
          className="fixed left-1/2 top-1/2 z-50 flex max-h-[88vh] w-[calc(100vw-2rem)] max-w-2xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-3xl border border-white/[0.08] bg-[var(--surface)] shadow-2xl shadow-black/70 outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in-0 data-[state=closed]:fade-out-0 data-[state=open]:zoom-in-95 data-[state=closed]:zoom-out-95"
        >
          {/* Header */}
          <div className="relative overflow-hidden border-b border-white/[0.06] bg-[var(--surface-2)]/40 px-6 py-6">
            ...
            <div className="relative flex items-start justify-between gap-4">
              ...
              <Dialog.Close asChild>
                <button
                  type="button"
                  data-testid="ticker-dialog-close"
            ...
            {position && (
              <div className="relative mt-5 grid gap-3 sm:grid-cols-3">
                {[
                  {
                    label: 'Cantidad',
                    value: useFormatNumber(position.quantity),
                    color: 'text-[var(--text)]',
                  },
                  {
                    label: 'Costo Prom.',
                    value: useMoney(position.avgCost),
                    color: 'text-[var(--text)]',
                  },
                  {
                    label: 'Total Operado',
                    value: useMoney(totalTraded),
                  ...
                  >
                    ...
                    <p
                      className={`mt-1.5 font-mono text-[18px] font-black ${color}`}
                    >
                      {value}
                    </p>
                  </div>
            ...
            {position && (
              <div className="mb-4 rounded-2xl border border-rose-400/20 bg-rose-500/[0.06] p-4">
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  <button
                    type="button"
                    data-testid="ticker-dialog-sell-all"
                    onClick={() => {
                      setSellSucceeded(false);
                ...
                {parsedSellQuantity > availableQuantity && (
                  <p className="mt-3 text-xs font-semibold text-rose-200">
                    Solo tenés {useFormatNumber(availableQuantity)} acciones
                    disponibles.
                  </p>
                )}
                ...
                {sellError && (
                  <p
                    data-testid="ticker-dialog-sell-error"
                ...
                {sellSucceeded && !sellError && (
                  <p
                    data-testid="ticker-dialog-sell-success"
                    className="mt-3 inline-flex items-center gap-2 text-xs font-semibold text-emerald-300"
                  >
                ...
                <div className="space-y-2">
                  {recentTransactions.map((transaction) => {
                    ...
                    return (
                      <div
                        key={transaction.id}
                        data-testid="ticker-dialog-tx-row"
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-[14px] font-black text-[var(--text)]">
                              {isBuy ? 'Compra' : 'Venta'} ·{' '}
                              {useFormatNumber(transaction.quantity)} acc.
                            </p>
