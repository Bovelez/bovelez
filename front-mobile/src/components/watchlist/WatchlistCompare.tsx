...
import {
  ...
  formatMetricValue,
  METRIC_ROWS,
} from '../../pages/watchlist/watchlist.utils.ts';
  ...
  return (
    <div data-testid="compare-section">
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              style={
                isSel ? { background: 'var(--gradient-brand)' } : undefined
              }
            >
              {item.ticker}
            </button>
      ...
      {!isPending && hasResults && allMetricsEmpty(results) && (
        ...
        >
          No hay métricas disponibles para ninguna de las empresas
          seleccionadas.
        </div>
      )}
        ...
        >
          ...
          >
            ...
            {results.map((r) => (
              <span
                key={r.name}
                data-testid="compare-col-header"
              ...
              {results.map((r) => {
                ...
                if (!latest) {
                  return (
                    <span
                      key={r.name}
                      data-testid="metric-nd"
                      className="text-xs font-bold text-[var(--text-faint)]"
                    >
                      N/D
                    </span>
                  );
                ...
                return (
                  <span
                    ...
                    className="px-2 py-1 rounded text-xs font-bold font-mono text-[var(--text)]"
                  >
