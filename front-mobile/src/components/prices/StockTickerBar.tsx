...
function StockTickerItem({ stock }: { stock: StockPrice }) {
  ...
  return (
    <li className="flex h-11 shrink-0 items-center gap-3 border-r border-white/[0.06] px-5">
      ...
      <span
        className={`inline-flex min-w-[72px] items-center gap-1 font-mono text-[11px] font-bold ${changeColor}`}
      >
        {hasChange &&
          (isPositive ? (
  ...
  return (
    <div className="relative flex-1 overflow-hidden" aria-hidden="true">
      ...
      >
        {items.map((stock, i) => (
          <StockTickerItem
            key={`${stock.ticker}-${stock.updatedAt}-${i}`}
            stock={stock}
          />
        ))}
      </ul>
  ...
  return (
    <div className="flex h-11 flex-1 items-center gap-5 overflow-hidden px-5">
      {Array.from({ length: 7 }).map((_, i) => (
        <span
          key={i}
          className="h-3.5 w-24 shrink-0 animate-pulse rounded-md bg-white/[0.06]"
        />
      ))}
    </div>
  );
