...
import type {SearchResponseDto, WatchlistItem, WatchlistMetricsResult} from "../types/watchlist.types";
...
export async function addWatchlistItem(ticker: string): Promise<WatchlistItem> {
    const { data } = await apiClient.post<WatchlistItem>("/watchlist", { ticker });
    return data;
}
...
export async function compareWatchlistMetrics(
    ...
    const { data } = await apiClient.post<WatchlistMetricsResult[]>("/watchlist/compare", { tickers });
    return data;
}
