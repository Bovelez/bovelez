...
export function WatchlistCompare({
    ...
    return (
        <div data-cy="compare-section">

            {/* Ticker chips */}
            <div data-cy="compare-selector" className="flex flex-wrap gap-2 mb-5">
                {items.length === 0 && (
            ...
            {!isPending && !hasResults && !isError && (
                <p data-cy="compare-hint" className="text-sm text-[var(--text-muted)]">
            ...
            {!isPending && hasResults && !allMetricsEmpty(results) && (
                ...
                >
                    ...
                    >
                        <span>Métrica</span>
                        {results.map((r) => (
                            <span key={r.name} data-cy="compare-col-header" className="font-mono text-[var(--primary)]">
                {r.name}
              </span>
                        ))}
                    </div>
                    ...
                    {METRIC_ROWS.map(({ label, key }, ri) => (
                        ...
                        >
                            ...
                            {results.map((r) => {
                                ...
                                if (!latest) {
                                    return (
                                        <span key={r.name} data-cy="metric-nd" className="text-xs font-bold text-[var(--text-faint)]">
                      N/D
                    </span>
                                    );
                                }
                                return (
                                    <span key={r.name} data-cy="metric-value" className="px-2 py-1 rounded text-xs font-bold font-mono text-[var(--text)]">
