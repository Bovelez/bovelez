        ...
        <span className="font-mono font-semibold text-[13px] text-[var(--primary)]">
          {item.ticker}
            ...
            <div className="text-right">
                {hasPrice ? (
                    <>
            <span data-cy="item-price" className="font-bold font-mono text-[var(--text)]">
              ${item.price!.toFixed(2)}
            </span>
                        {item.priceUpdatedAt && (
                            <p data-cy="item-price-date" className="text-[10px] text-[var(--text-faint)]">
                                {new Date(item.priceUpdatedAt).toLocaleString()}
                            </p>
                        )}
                    </>
                ) : (
                    <span data-cy="item-no-price" className="text-xs text-[var(--text-faint)]">
            Sin precio
          </span>
                )}
            </div>
