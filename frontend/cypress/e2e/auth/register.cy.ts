describe("Register Page", () => {
    ...
    it("shows an error when passwords do not match", function () {
        ...
        cy.getByTestId("confirm-password-input").should("have.class", "border-[var(--danger)]");
    });
    ...
    it("displays a global error alert when the API returns an error", function () {
        // El usuario ya existe en la DB reseteada (seed), así que el server devuelve 409
        cy.fillRegisterForm({ nombre: this.u.nombre, email: this.u.email, password: this.u.password });
